...
## 개발 & 배포 방식 (단일 `gh-pages` 브랜치)

이 저장소는 **`gh-pages` 브랜치 하나로 통일**해서 운영합니다.
소스코드(루트)와 빌드 결과(`docs/`)가 같은 브랜치에 공존하며, GitHub Pages는
`gh-pages` 브랜치의 **`/docs` 폴더**를 서빙합니다.

```
gh-pages 브랜치 (유일한 작업/배포 브랜치)
├── src/ · index.html · vite.config.ts …   ← 소스코드 (루트)
└── docs/                                   ← 빌드 결과 (GitHub Pages가 서빙)
```
...
3. 배포 시 빌드 후 `docs/`까지 함께 커밋·푸시

```bash
npm run deploy   # = npm run build → docs 빌드 → 자동 커밋·푸시
# (또는 수동) npm run build && git add docs && git commit -m "build" && git push
```
...
- **Settings → Pages → Build and deployment**
  ...
  - Branch: **`gh-pages`** / 폴더: **`/docs`**
- 커스텀 도메인(`ai-free.dreamitbiz.com`)은 `public/CNAME`(→ 빌드 시 `docs/CNAME`)로 관리됩니다.
- `public/.nojekyll` 로 Jekyll 처리를 비활성화해 `assets/` 등이 그대로 서빙됩니다.

> 빌드 출력 폴더는 `vite.config.ts`의 `build.outDir = 'docs'` 입니다.
